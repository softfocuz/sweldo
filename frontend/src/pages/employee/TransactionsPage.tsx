...
import { listAssets, listClaimsForEmployee, listPayrolls, listTransactions } from "@/lib/api";
import { getErrorMessage } from "@/lib/errors";
import { stellarExpertTxUrl } from "@/lib/stellar";
import { useAuth } from "@/context/AuthContext";
import { formatAmount, formatDate } from "@/lib/format";
import type { Payroll, TransactionRecord } from "@/types";
...
export function TransactionsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setLoadError(null);
      try {
        const [claimsRes, payrollsRes, assetsRes, transactionsRes] = await Promise.all([
        ...
        const payrollById = new Map(payrollsRes.data.map((p) => [p.id, p]));
        const assetCodeById = new Map(assetsRes.data.map((a) => [a.id, a.code]));
        const claimById = new Map(claimsRes.data.map((c) => [c.id, c]));
  ...
  return (
    <DashboardLayout role="employee" title="Transactions">
      <Card>
        <CardHeader>
          <h2 className="font-display text-base font-semibold text-ink-900">Claimed payments</h2>
          <p className="mt-1 text-sm text-ink-500">Payments you've claimed from your payrolls.</p>
        </CardHeader>
